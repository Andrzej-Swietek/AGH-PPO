...
int main(int argc, char** argv) 
{
	...
	using CompareFunction = void (*)(int a, int b, Functor& fun);
	using FunctionsMap = std::map<std::pair<int,int>, CompareFunction>;
	...
	std::for_each(functions.begin(), functions.end(), [&](const auto& entry) {
        CompareFunction compareFunc = entry.second;
        std::pair<int, int> parameters = entry.first; 

        compareFunc(parameters.first, parameters.second, f);
    });
